...
- **紹介コードの薄い表示（2026-07-27追加）**: 登録LP（`agency_register/landing.blade.php`）の`.lp-footer`と同じ見た目（`text-gray-400`・11px・中央寄せ）で、ページ最下部に`招待リンクの紹介元パートナー`の`referral_code`（`legacy_code`）を常時表示。入力欄ではなく読み取り専用の表示のみ。目的はお客様向けというより、**パートナーが「フォームを確認」ボタンで自分の招待リンクをプレビューした時に、ちゃんと自分の紹介として認識される見た目にする**ための安心材料（LPの「コードは見せるが名前は見せない」プライバシー方針を踏襲）。`show()`/`completeInquiry()`双方の`view()`呼び出しに`referralCode' => $inviteLink->agency->referral_code`を渡している
