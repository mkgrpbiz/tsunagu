...
<div class="bg-white border border-gray-200 rounded-lg p-6 mb-6">
    <p class="text-xs text-gray-500 mb-3">
        「日付 - 紹介コード - 名前 - フリガナ - 商品名 - 金額」の順にタブ区切りで貼り付けてください。1行1件です。日付は参考表示のみです。紹介コードが「SHAREPOY」の行は名前・フリガナでSharePoy+管理のユーザーを検索して紐付け、「SP」から始まる行はそのままコードとして扱います。同じ紹介コードは合算し、件数×300ポイント（BIMONI紹介）としてコピー用一覧に表示します。金額はA01（シェアポイ）への一括着金紐付けで実際に使用します（金額ごとに件数をまとめてTSUNAGU側のContractを作成）。
    </p>
