...
<p class="text-xs text-gray-500 mb-6">商品受け取りモニター・覆面調査モニターでA01(シェアポイ)処理された着金のうち、まだSharePoy+ユーザーの履歴に記録していない分を名前で実施ユーザーに紐付け、着金履歴に記録します。紹介ポイント(件数×300pt)は実施ユーザー本人ではなく、その人をSharePoy+に紹介した人(紹介コード)に付与するため、コピー用一覧は紹介コード側で作成します。確定すると記録済みになり、次回以降は対象外になります。</p>
...
@foreach ([['data' => $productMonitor, 'title' => '商品受け取りモニター', 'route' => 'admin.sharepoy-points.store-product-monitor'], ['data' => $mysteryShopper, 'title' => '覆面調査モニター', 'route' => 'admin.sharepoy-points.store-mystery-shopper']] as $section)
    ...
    <div class="bg-white border border-gray-200 rounded-lg p-6 mb-4">
        <p class="text-sm text-gray-700 mb-1">対象: {{ count($section['data']['groups']) + count($section['data']['noReferrerCode']) }}件</p>
        @if (count($section['data']['noReferrerCode']) > 0)
            <p class="text-sm text-amber-600">紹介コード未登録(ポイント付与対象外): {{ count($section['data']['noReferrerCode']) }}件</p>
        @endif
    ...
    @if (count($section['data']['groups']) + count($section['data']['noReferrerCode']) > 0)
        <div class="bg-white border border-gray-200 rounded-lg overflow-hidden mb-4">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @endforeach
                    @foreach ($section['data']['noReferrerCode'] as $group)
                        <tr class="even:bg-gray-50 hover:bg-gray-100">
                            <td class="px-4 py-2">
                                <a href="{{ route('admin.sharepoy-users.show', $group['sharePoyUser']) }}" class="text-blue-600 hover:underline">{{ $group['sharePoyUser']->sharepoy_user_id }}</a>
                            </td>
    ...
    <form method="POST" action="{{ route($section['route']) }}" class="mb-10">
        @csrf
        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-md px-4 py-2" @disabled(count($section['data']['groups']) === 0 && count($section['data']['noReferrerCode']) === 0 && count($section['data']['unmatched']) === 0)>
            確定して履歴に記録
        </button>
    </form>
@endforeach
