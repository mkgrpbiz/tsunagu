...
@section('title', '覆面調査モニター - 一括紐付けプレビュー')
                ...
                @foreach ($matched as $item)
                    @foreach ($item['lines'] as $line)
                        <tr>
                            ...
                            <td class="px-4 py-2">{{ $item['inquiry']->agency?->name }}</td>
...
<form method="POST" action="{{ route('admin.mystery-shopper-links.bulk-store') }}" class="flex gap-3">
    @csrf
    ...
    <a href="{{ route('admin.mystery-shopper-links.index') }}" class="text-sm text-gray-500 px-4 py-2">戻る</a>
</form>
@endsection
