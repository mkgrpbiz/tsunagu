...
@section('title', '一括追加プレビュー')

@section('content')
<div class="flex items-center justify-between mb-6">
    <h1 class="text-xl font-semibold">一括追加プレビュー</h1>
    <a href="{{ route('admin.agencies.index') }}" class="text-sm text-blue-600 hover:underline">戻る</a>
</div>
...
<div class="bg-white border border-gray-200 rounded-lg p-4 mb-6">
    ...
    <p class="text-xs text-gray-500 mt-1">内容を確認のうえ、問題なければ下の「この内容で一括追加する」を押してください。エラーの行は追加されません。初期パスワードは全員 pass1234 です。</p>
</div>
        ...
        <table class="w-full text-sm min-w-max">
            <thead class="bg-gray-50 text-gray-500 text-left">
                <tr>
                    ...
                    <th class="px-4 py-3 font-medium">紹介元</th>
                    ...
                    <th class="px-4 py-3 font-medium">ご職業</th>
                    <th class="px-4 py-3 font-medium">電話番号</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @foreach ($valid as $row)
                    <tr>
                        ...
                        <td class="px-4 py-3">
                            @if ($row['referrer_agency'])
                                {{ $row['referrer_agency']->name }}（{{ $row['referrer_agency']->referral_code }}）
                            @elseif ($row['referrer_in_batch'])
                                <span class="text-xs font-medium border rounded-full px-1.5 py-0.5 bg-blue-50 text-blue-700 border-blue-200">この貼り付け内の別行（{{ $row['referral_code'] }}）</span>
                            @elseif ($row['referral_code'] !== '')
                                <span class="text-xs font-medium border rounded-full px-1.5 py-0.5 bg-amber-50 text-amber-700 border-amber-200">該当なし（{{ $row['referral_code'] }}）</span>
                            @else
                                —
                            @endif
                        </td>
                        ...
                        <td class="px-4 py-3">{{ $row['prefecture'] }}</td>
                        <td class="px-4 py-3">{{ $row['occupation'] ?? '—' }}</td>
...
@if (count($valid) > 0)
    <form method="POST" action="{{ route('admin.agencies.bulk-store') }}" onsubmit="return confirm('{{ count($valid) }}件のパートナーを追加します。よろしいですか？');">
        @csrf
